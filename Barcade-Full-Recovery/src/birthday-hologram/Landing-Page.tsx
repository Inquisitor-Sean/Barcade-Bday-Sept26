...
export default function LandingPage() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('after-hours-lock-appearance', JSON.stringify(appearance))
    } catch {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setPhase('open'), paused ? 0 : 2600)
  ...
  return (
    <div
      ref={surfaceRef}
      data-invite-version="circular-sand-lock-v3"
      style={style}
    ...
    >
      ...
      {phase === 'open' ? (
        ...
        <main className="landing-page lock-landing">
          <button
            type="button"
            ref={opener}
            className="lock-trigger"
            disabled={phase === 'opening'}
            aria-label={t(phase === 'opening' ? 'decrypting' : 'decrypt')}
            aria-busy={phase === 'opening'}
            onClick={() => setPhase(paused ? 'open' : 'opening')}
          >
            {appearance.contrast && (
              <svg className="lock-wire" viewBox="-1.125 -1.125 2.25 2.25" aria-hidden="true">
                <circle r=".98" />
                <path d="M.155 -.1a.155.155 0 1 0-.31 0 .155.155 0 0 0 .31 0ZM-.07 .04V.32H.07V.04" />
              </svg>
            )}
          </button>
          <span className="sr-only" role="status">{phase === 'opening' ? t('decrypting') : ''}</span>
        </main>
      )}
      {phase === 'open' && <>
        <footer className="experience-footer">
          <div className="motion-control">{motionButton}</div>
        </footer>
        <aside className={`event-update${cancelled ? ' is-cancelled' : ''}`} role={cancelled ? 'alert' : 'status'}>
          {cancelled ? <><strong>{t('cancelled')}</strong><span>{status.message || t('cancelledNote')}</span></> :
            <span>{status.error ? t('statusError') : status.message || t('statusNote')}</span>}
          {(cancelled || status.error) && <a href={event.backupUrl} target="_blank" rel="noopener noreferrer">{t('backup')} ↗</a>}
        </aside>
      </>}
